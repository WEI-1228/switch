...
def define_components(mod):
    ...
    mod.gen_min_uptime = Param(
        mod.GENERATION_PROJECTS, within=NonNegativeReals, default=2
    )
    ...
    mod.COAL_BASED_GENS = Set(
        dimen=1,
        initialize=mod.GENERATION_PROJECTS,
        filter=lambda m, g: m.gen_energy_source[g]== "Coal"
    )
        
    mod.gen_climb_rate = Param(
        mod.COAL_BASED_GENS,
        within=NonNegativeReals,
        default=0.2
    )
    mod.COAL_BASED_GEN_TPS = Set(
        dimen=2,
        initialize=lambda m: (
            (g, tp) for g in m.COAL_BASED_GENS for tp in m.TPS_FOR_GEN[g]
        ),
    )    
    mod.ClimbingPowerOfUp= Constraint(
        mod.COAL_BASED_GEN_TPS,
        rule=lambda m, g, t: ((m.DispatchGen[g, t]- m.DispatchGen[g, m.tp_previous[t]]) 
        <= m.gen_climb_rate[g] * m.CommitGen[g, t]),
    )
    mod.ClimbingPowerOfDown= Constraint(
        mod.COAL_BASED_GEN_TPS,
        rule=lambda m, g, t: ( - m.gen_climb_rate[g] * m.CommitGen[g, t]
        <=(m.DispatchGen[g, t]- m.DispatchGen[g, m.tp_previous[t]])),
    )
